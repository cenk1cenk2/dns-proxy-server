...
docker-push)
  ...
  docker tag cenk1cenk2/dns-proxy-server:${APP_VERSION} cenk1cenk2/dns-proxy-server:nightly &&
    docker tag cenk1cenk2/dns-proxy-server:${APP_VERSION} cenk1cenk2/dns-proxy-server:unstable &&
    echo "$DOCKER_PASSWORD" | docker login -u "$DOCKER_USERNAME" --password-stdin &&
    docker-compose push image-linux-amd64 &&
    docker push cenk1cenk2/dns-proxy-server:nightly &&
    docker push cenk1cenk2/dns-proxy-server:unstable
  echo "Push done"
  ;;
...
deploy-docs)
  ...
  {
    git ls-tree origin/gh-pages | grep -E -o '[0-9]+\.[0-9]+'
    echo "${MINOR_VERSION}"
  } |
    sort -V -r |
